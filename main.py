...
from src.core.crypto_pipeline import CryptoAnalysisPipeline
...
def run_full_analysis(
    ...
    try:
        ...
        # 3. 币圈分析（实验性）：与股票链路解耦，作为额外日报追加执行
        try:
            if getattr(config, "crypto_enabled", False) and getattr(config, "crypto_symbol_list", []):
                logger.info("[Crypto] 检测到币圈配置，开始执行币圈轻量分析流程")
                crypto_pipeline = CryptoAnalysisPipeline(
                    config=config,
                    analyzer=pipeline.analyzer,
                    search_service=pipeline.search_service,
                    notifier=pipeline.notifier,
                ...
                crypto_pipeline.run(
                    dry_run=args.dry_run,
                    send_notification=not args.no_notify,
                    report_type=ReportType.from_str(getattr(config, "report_type", "simple")),
                )
        except Exception as e:
            logger.warning(f"[Crypto] 币圈分析流程失败（已忽略，不影响股票链路）: {e}")
