 ...
 设计目标（最小可用版本）：
 1. 复用现有 LLM 分析器（GeminiAnalyzer）、搜索服务（SearchService）和通知服务（NotificationService）；
 2. 使用 Binance 公共 API 获取币种 K 线与 24h 行情（通过 data_provider.crypto_binance）；
 3. 每日对一批配置好的币种（CRYPTO_SYMBOL_LIST）生成简化版「决策仪表盘」，并通过已有渠道推送；
 4. 不改动现有股票分析链路，币圈分析作为额外链路追加执行。
 """
 ...
 class CryptoAnalysisPipeline:
     ...
     def _build_crypto_context(self, symbol: str) -> Optional[CryptoContext]:
         ...
         ctx = CryptoContext(
             symbol=symbol,
             date=today_row["date"].strftime("%Y-%m-%d") if isinstance(today_row.get("date"), datetime) else str(
                 today_row.get("date")
             ),
             today=today_row,
     ...
     def _convert_to_stock_like_context(self, ctx: CryptoContext) -> Dict[str, Any]:
         ...
         # 价格涨跌与量能变化（复用股票逻辑）
         try:
             if yesterday and today.get("close") is not None and yesterday.get("close") not in (None, 0):
         ...
         try:
             if yesterday and today.get("volume") is not None and yesterday.get("volume") not in (None, 0):
     ...
     def run(
         ...
         try:
             rt = report_type or ReportType.from_str(getattr(self.config, "report_type", "simple"))
             report_md = self.notifier.generate_aggregate_report(results, rt, report_date=None)
