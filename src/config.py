...
class Config:
    ...
    # === 币圈配置（实验性）===
    # 是否启用币圈分析（CRYPTO_ENABLED=true 时启用）
    crypto_enabled: bool = False
    # 币种列表（如 BTCUSDT,ETHUSDT,SOLUSDT），从 CRYPTO_SYMBOL_LIST 解析
    crypto_symbol_list: List[str] = field(default_factory=list)
    # 币圈行情数据源标识（预留，后续用于选择 binance/coingecko 等具体实现）
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # === 币圈配置解析（实验性）===
        # 是否开启币圈分析（与股票共存），默认关闭保持零回归
        crypto_enabled = os.getenv('CRYPTO_ENABLED', 'false').strip().lower() == 'true'
        # 币种列表，逗号分隔，统一转为大写并去掉空项
        crypto_symbol_list_str = os.getenv('CRYPTO_SYMBOL_LIST', '')
        crypto_symbol_list = [
            (c or '').strip().upper()
            for c in crypto_symbol_list_str.split(',')
            if (c or '').strip()
        ]
        # 币圈行情数据源标识，预留给后续的数据源适配层（如 binance/coingecko）
        crypto_data_provider = os.getenv('CRYPTO_DATA_PROVIDER', 'binance').strip().lower() or 'binance'
        
        return cls(
            stock_list=stock_list,
            crypto_enabled=crypto_enabled,
            crypto_symbol_list=crypto_symbol_list,
            crypto_data_provider=crypto_data_provider,
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        # --- Crypto config (experimental) ---
        # 当显式开启 CRYPTO_ENABLED 时，要求至少配置一个币种，避免误开启空任务
        if getattr(self, "crypto_enabled", False) and not getattr(self, "crypto_symbol_list", []):
            issues.append(ConfigIssue(
                severity="warning",
                message="已开启币圈分析 (CRYPTO_ENABLED=true)，但未配置任何币种 (CRYPTO_SYMBOL_LIST)，当前不会执行币圈分析",
                field="CRYPTO_SYMBOL_LIST",
            ))
